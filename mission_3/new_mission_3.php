...
<?php
	...
	if (isset($_POST["name"],$_POST["comment"],$_POST["passwd"])) {
...
//	}else{
		...
		for ($i=0; $i < count($del_con); $i++) { 
			...
			if ($del_num[0] != $delete){
				//入力データのファイル書き込み
				fwrite($fp, $del_con[$i]);
			}else if ($del_num[0] == $delete){ 

			//パスワードがあっているか
			if($del_num[4] != $_POST["del_passwd"]){
				echo "パスワードが違います。<br><br>";
				fwrite($fp, $del_con[$i]);
			}else if($del_num[4] == $_POST["del_passwd"]){
				$have_del = $del_num[0];
			}
		}
		}
...
<?php
	...
	//fopもexceptも配列になった！！！
	//explode関数でそれぞれの値を取得
	$cnt_ex = count($except = explode("<>",$fop[$i])) -2;
